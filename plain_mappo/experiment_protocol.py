...
def write_locked_seed_manifest(path: Path) -> tuple[dict[str, Any], str, bool]:
    ...
    return proposed, sha256_file(path), True
...
def load_seed_manifest(path: Path) -> tuple[dict[str, Any], str]:
    ...
    validation = tuple(int(seed) for seed in payload["splits"]["validation"]["seeds"])
    test = tuple(int(seed) for seed in payload["splits"]["test"]["seeds"])
    if len(validation) != 20 or len(test) != 50 or set(validation).intersection(test):
